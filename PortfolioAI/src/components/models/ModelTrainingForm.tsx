...
const ACCEPTED_ZIP_FILES= ["application/x-zip-compressed","aplication/zip"]
const MAX_FILE_SIZE= 47*1024*1024 //45MB
...
const formSchema = z.object({
  ...
  zipFile: z.any().refine(files => files?.[0] instanceof File, "Please select a valid file")
  .refine(
    (files) => files?.[0]?.type && ACCEPTED_ZIP_FILES.includes(files?.[0]?.type),
  "Only zip files are accepted!").refine((files) => files?.[0]?.size <= MAX_FILE_SIZE, "max file size allowed is 45 mb."), 
});
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
      ...
      try{
       ...
       const urlResponse = await fetch(data.signedUrl,{
        ...
        toast.loading("Initialting model traning...",{id:toastId})
        // /Train handler
        ...
        toast.success("Training started successfully! You will receive a notification once it gets completed!", {id: toastId})
          ...
          <fieldset className="grid max-w-5xl bg-background p-8 rounded-lg gap-6 border">
            <FormField
                ...
                <FormItem>
                    ...
                    <FormDescription>
                    This wil be the name of your trained model.
                    </FormDescription>
                    <FormMessage />
                </FormItem>
                )}
            ...
            <FormField
                ...
                <FormItem>
                    ...
                    <FormDescription>
                    Upload a zip file containing you training image (45mb)
                    </FormDescription>
                    <FormMessage />
                </FormItem>
                )}
