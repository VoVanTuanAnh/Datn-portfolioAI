...
export async function deleteModel(id: number, model_id: string, model_version: string) {
    ...
    if(model_version){
        try{
            const res = await fetch(`https://api.replicate.com/v1/models/vovantuananh/${model_id}/versions/${model_version}`,{
                method: 'DELETE',
                headers:{
            ...
            console.error('Failed to detede model version from replicate: ', e)
    ...
    if(model_id){
        try{
            const res = await fetch(`https://api.replicate.com/v1/models/vovantuananh/${model_id}`,{
                method: 'DELETE',
                headers:{
                    'Authorization':`Bearer ${process.env.REPLICATE_API_TOKEN}`
                }
            ...
            console.error('Failed to detede model from replicate: ', e)
    ...
    return{
        error: error?.message || 'Failed to delete model version from replicate',
        success: !error,
    }
}